  ...
  describe 'タイトルに' do
    ...
    context '半角数字が含まれる場合' do
      ...
      it '半角数字で検索してヒットすること' do
        expect(Post.ransack({ title_translate_cont_any: '999' }).result).to be_exists
      end
      it '全角数字で検索してヒットすること' do
        expect(Post.ransack({ title_translate_cont_any: '９９９' }).result).to be_exists        
      end
    end
    ...
    context '半角英字が含まれる場合' do
      ...
      it '半角英字で検索してヒットすること' do
        expect(Post.ransack({ title_translate_cont_any: 'Docker' }).result).to be_exists
      end
      it '全角英字で検索してヒットすること' do
        expect(Post.ransack({ title_translate_cont_any: 'Ｄｏｃｋｅｒ' }).result).to be_exists
      end
    end
    ...
    context '半角記号が含まれる場合' do
      ...
      it '半角記号で検索してヒットすること' do
        expect(Post.ransack({ title_translate_cont_any: '集まれ!' }).result).to be_exists
      end
